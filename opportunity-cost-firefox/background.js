...
browser.browserAction.onClicked.addListener(() => {
  ...
  browser.tabs.create({
    url: "https://tftc.io/bitcoin-brief?utm_source=opportunity-cost-extension",
  });
});
...
const PriceDatabase = {
  ...
  saveBitcoinPrice: async (currency, price) => {
    ...
    const history = (await browser.storage.local.get("price_history")) || {
      price_history: [],
    };
  ...
  saveVisitedSite: async (url, conversionCount) => {
    const timestamp = Date.now();
    const sites = (await browser.storage.local.get("visited_sites")) || {
      visited_sites: {},
    };
    ...
    sites.visited_sites[url] = {
      ...
      timestamp: timestamp,
      conversionCount:
        (sites.visited_sites[url]?.conversionCount || 0) + conversionCount,
      date: new Date(timestamp).toISOString(),
    };
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "getBitcoinPrice") {
    responsePromise = getBitcoinPrice()
      .then((price) => {
        return {
          price: price,
    ...
    if (userPreferences?.trackStats !== false) {
      responsePromise = PriceDatabase.saveVisitedSite(
        message.url,
    ...
    responsePromise = loadUserPreferences()
      .then(() => {
        return { success: true };
      })
      .catch((error) => {
